...
LSTM_UNITS_2  = 64
DROPOUT_RATE  = 0.3
LEARNING_RATE = 0.01          # initial LR (adaptive scheduler will adjust)
LR_MIN        = 1e-6           # floor for adaptive decay
WARMUP_EPOCHS = 5              # linear warmup before adaptive kicks in
# PATIENCE      = 10
PATIENCE      = 30
...
# Linear warmup → cosine annealing (adaptive decay)
def lr_lambda(epoch):
    if epoch < WARMUP_EPOCHS:
        # linear warmup: ramp from ~0 to LEARNING_RATE
        return (epoch + 1) / WARMUP_EPOCHS
    # cosine decay from LEARNING_RATE down to LR_MIN
    progress = (epoch - WARMUP_EPOCHS) / max(1, EPOCHS - WARMUP_EPOCHS)
    cosine_decay = 0.5 * (1 + np.cos(np.pi * progress))
    return max(LR_MIN / LEARNING_RATE, cosine_decay)
