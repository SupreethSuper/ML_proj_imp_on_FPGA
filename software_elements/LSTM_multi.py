...
OUTPUT_DIR   = os.path.join(SCRIPT_DIR, "lstm_multi")
...
axes[0].set_title("LSTM Multi-Class Classifier - Confidence Scores Over Time (Test Set)",
                   fontsize=14)
...
ax2.set_title("LSTM Multi-Class - Stacked Confidence Scores (Test Set)", fontsize=14)
...
for cls_idx, (ax, name, color) in enumerate(zip(axes3, CLASS_NAMES, CLASS_COLORS)):
    ...
    ax.grid(True, alpha=0.3)
fig3.suptitle("Confidence Score Distributions by True Class", fontsize=14, y=1.02)
fig3.tight_layout()
...
ax4.set_title("ROC Curves (One-vs-Rest) - LSTM Multi-Class", fontsize=14)
...
ax5.set_title("Training & Validation Loss", fontsize=14)
...
ax6.set_title("Train Accuracy vs Test Accuracy", fontsize=14)
...
ax7.set_title("Confusion Matrix - LSTM Multi-Class", fontsize=14)
# annotate cells
...
ax8.set_title("Max Confidence per Sample (colored by true class)", fontsize=14)
...
# =====================================================================
torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, "lstm_multi_model.pth"))
print(f"\n  Model saved: lstm_multi_model.pth")
